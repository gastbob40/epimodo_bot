...
async def add_warn(client: discord.Client, message: discord.Message, args: List[str]):
    ...
    await message.channel.send(
        embed=EmbedsManager.complete_embed(f"You just warned {target.display_name} for {reason}.")
    )

    await LogManager.complete_log(client, 'warns', message.author, message.guild, reason, image_url)

    try:
        await target.send(
            embed=EmbedsManager.sanction_embed('Warn', message.guild, reason, image_url)
        )
