...
async def add_kick(client: discord.Client, message: discord.Message, args: List[str]):
    ...
    if args and args[0] == '-h':
        return await message.channel.send(
            embed=EmbedsManager.information_embed("**Kicking command reminder:**\n\n"
                                                  "`!kick <@user> <reason>`.\n"
    ...
    if target_results >= results:
        return await message.channel.send(
            embed=EmbedsManager.error_embed("You cannot kick someone greater than or equal to you.")
        )
    ...
    await message.channel.send(
        embed=EmbedsManager.complete_embed(f"You just kicked {target.display_name} for {reason}.")
    )

    await LogManager.complete_log(client, 'kicks', message.author, message.guild, reason, image_url)

    try:
        await target.send(
            embed=EmbedsManager.sanction_embed('Kick', message.guild, reason, image_url)
        )
        await target.kick(reason=reason)
    except:
        await message.channel.send(
            embed=EmbedsManager.error_embed("Error in kicking the member.")
        )
