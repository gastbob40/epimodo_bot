...
async def add_jail(client: discord.Client, message: discord.Message, args: List[str]):
    ...
    if args and args[0] == '-h':
        return await message.channel.send(
            embed=EmbedsManager.information_embed("**Jailing command reminder:**\n\n"
                                                  "`!jail <@user> <reason>`.\n"
    ...
    await LogManager.complete_log(client, 'mutes', message.author, message.guild, reason, image_url)
