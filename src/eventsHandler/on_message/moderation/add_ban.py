...
async def add_ban(client: discord.Client, message: discord.Message, args: List[str]):
    ...
    if args and args[0] == '-h':
        return await message.channel.send(
            embed=EmbedsManager.information_embed("**Banning command reminder:**\n\n"
                                                  "`!ban <@user> <reason>`.\n"
    ...
    if target_results >= results:
        return await message.channel.send(
            embed=EmbedsManager.error_embed("You cannot ban someone greater than or equal to you.")
        )
    ...
    await message.channel.send(
        embed=EmbedsManager.complete_embed(f"You just banned {target.display_name} for {reason}.")
    )

    await LogManager.complete_log(client, 'bans', message.author, message.guild, reason, image_url)

    try:
        await target.send(
            embed=EmbedsManager.sanction_embed('Ban', message.guild,reason, image_url)
        )
